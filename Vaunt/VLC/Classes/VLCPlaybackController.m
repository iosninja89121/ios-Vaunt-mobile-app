...
- (void)_updateStoredThumbnailForFile:(MLFile *)fileItem
{
    ...
    @try {
        [fileItem setComputedThumbnail:[UIImage scaleImage:newThumbnail
...
{
    return [_mediaPlayer titles].count > 1 || [_mediaPlayer chaptersForTitleIndex:_mediaPlayer.currentTitleIndex].count > 1;
}
...
- (void)_updateDisplayedMetadata
{
    ...
    if (mediaIsAudioOnly) {
        ...
        if (item) {
            ...
            if (_mediaPlayer.videoSubTitlesIndexes.count > 2) {
...
- (void)applicationDidBecomeActive:(NSNotification *)notification
{
    ...
    if (_mediaPlayer.videoTrackIndexes.count > 0) {
