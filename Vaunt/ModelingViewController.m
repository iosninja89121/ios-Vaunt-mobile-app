...
@interface ModelingViewController ()
{
    NSArray *sections;
...
@implementation ModelingViewController
...
- (void)viewDidLoad {
    ...
    sections = [[[DBUtil sharedInstance] channelsArray] objectAtIndex:1];
...
- (void)onbtnPlayLongClicked:(UILongPressGestureRecognizer *)gestureRecognizer
{
    ...
    NSMutableArray* arrayWatchList = [userDefaults objectForKey:@"watchlist"];
    if (arrayWatchList == nil){
        arrayWatchList = [NSMutableArray array];
        [userDefaults setObject:arrayWatchList forKey:@"watchlist"];
    }
    
    bool bExist = false;
    for(int i = 0; i < arrayWatchList.count; i++){
        NSInteger tmpKey = [arrayWatchList[i] integerValue];
        if(tmpKey == tag){
            bExist = true;
            break;
        }
    ...
    if(bExist == false){
        [arrayWatchList addObject:[NSNumber numberWithInteger:tag ]];
    }
